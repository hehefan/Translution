...
class Attention(nn.Module):
    def __init__(self, hw_size, dim, heads = 8, dim_head = 64, ratio = 16, dropout = 0.):
        ...
        self.to_v1 = nn.Linear(dim, inner_dim//ratio, bias = False)
        self.to_v2 = SharedParameter(height, width, inner_dim//ratio, inner_dim//ratio)
        self.to_v3 = nn.Linear(inner_dim//ratio, inner_dim, bias = False)
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels = 3, dim_head = 64, ratio = 16, dropout = 0., emb_dropout = 0.):
        super().__init__()
