...
class Attention(nn.Module):
    def __init__(self, hw_size, dim, heads = 8, dim_head = 64, ratio = 16, dropout = 0.):
        super().__init__()
        ...
        mid_dim = inner_dim//ratio
        self.to_v1 = nn.Linear(dim, mid_dim, bias = False)
        self.to_v2 = SharedParameterRelCls(height, width, mid_dim, mid_dim)
        self.to_v3 = nn.Linear(mid_dim, inner_dim, bias = False)
    ...
    def forward(self, x):
        ...
        w = self.to_v2().unsqueeze(0)                                   # 1 n n dim C
        v2 = torch.matmul(v2, w).squeeze(3)                             # b n n C
...
class Translution(nn.Module):
    def __init__(self, hw_size, dim, depth, heads, dim_head, mlp_dim, ratio = 16, dropout = 0.):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Attention(hw_size, dim, heads = heads, dim_head = dim_head, ratio = ratio, dropout = dropout),
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, ratio = 16, dropout = 0., emb_dropout = 0.):
        super().__init__()
        ...
        self.translution = Translution((image_height // patch_height, image_width // patch_width), dim, depth, heads, dim_head, mlp_dim, ratio, dropout)
...
def alution_vit_mini(image_size = 224, patch_size = 16, ratio = 16, num_classes = 1000, pool = 'cls'):
...
def alution_vit_tiny(image_size = 224, patch_size = 16, ratio = 16, num_classes = 1000, pool = 'cls'):
    return ViT(image_size = image_size,
               patch_size = patch_size,
               num_classes = num_classes,
               dim = 192,
...
def alution_vit_small(image_size = 224, patch_size = 16, ratio = 16, num_classes = 1000, pool = 'cls'):
    return ViT(image_size = image_size,
               patch_size = patch_size,
               num_classes = num_classes,
               dim = 384,
...
def alution_vit_base(image_size = 224, patch_size = 16, ratio = 16, num_classes = 1000, pool = 'cls'):
    return ViT(image_size = image_size,
               patch_size = patch_size,
               num_classes = num_classes,
               dim = 768,
...
def alution_vit_large(image_size = 224, patch_size = 16, ratio = 16, num_classes = 1000, pool = 'cls'):
    return ViT(image_size = image_size,
               patch_size = patch_size,
               num_classes = num_classes,
               dim = 1024,
