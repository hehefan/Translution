...
class Translution(nn.Module):
    def __init__(self, hw_size, dim, heads = 8, dim_head = 64, pool = 'cls', dropout = 0.):
        ...
        self.to_v = SharedParameterRelCls(self.height, self.width, dim, inner_dim)
        ...
        if tln_num is not None: # hybrid
            ...
            for _ in range(tln_num):
                self.layers.append(nn.ModuleList([
                    Translution(hw_size, dim, heads = heads, dim_head = dim_head, pool = pool, dropout = dropout),
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., tln_num = None, pos_embedding = False):
        super().__init__()
        ...
        self.translution = TNN((image_height // patch_height, image_width // patch_width), dim, depth, heads, dim_head, mlp_dim, pool, dropout, tln_num)
...
def rope_tiny(image_size = 224, patch_size = 16, num_classes = 1000, tln_num = None):
...
def rope_mini(image_size = 224, patch_size = 16, num_classes = 1000, tln_num = None):
    return ViT(image_size = image_size,
               patch_size = patch_size,
               num_classes = num_classes,
               dim = 192,
...
def rope_small(image_size = 224, patch_size = 16, num_classes = 1000, tln_num = None):
    return ViT(image_size = image_size,
               patch_size = patch_size,
               num_classes = num_classes,
               dim = 384,
...
def rope_base(image_size = 224, patch_size = 16, num_classes = 1000, tln_num = None):
    return ViT(image_size = image_size,
               patch_size = patch_size,
               num_classes = num_classes,
               dim = 768,
...
def rope_large(image_size = 224, patch_size = 16, num_classes = 1000, tln_num = None):
    return ViT(image_size = image_size,
               patch_size = patch_size,
               num_classes = num_classes,
               dim = 1024,
