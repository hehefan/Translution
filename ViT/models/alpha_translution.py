...
class SharedParameterRelCls(nn.Module):
    def __init__(self, height, width, in_dim, out_dim):
        super().__init__()
        num_img_weights = (2*height-1) * (2*width-1)
        idx_cls = num_img_weights
        idx_cls_in = num_img_weights + 1 
        idx_cls_out = num_img_weights + 2 
        self.unique_params = nn.Parameter(torch.empty(num_img_weights + 3, in_dim, out_dim))
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dim_relenc = 16, dropout = 0., emb_dropout = 0.):
        super().__init__()
