...
class PreloadImageFolder(Dataset):
    def __init__(self, root, transform=None, as_tensor=False, num_workers=16):
        self.inner = ImageFolder(root)
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            results = executor.map(load_item, self.inner.samples)

        for img, target in results:
    ...
    def __getitem__(self, index):
        ...
        # If stored as PIL, still allow transform at runtime
        if not self.as_tensor and self.transform:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dummy:
        ...
        train_dataset = PreloadImageFolder(
            traindir,
            transforms.Compose([
        ...
        val_dataset = PreloadImageFolder(
            valdir,
            transforms.Compose([
